...
const FavoriteBooks = () => {
    ...
    return (
        <div>
            <h1 className="text-3xl text-themeColor font-bold pl-10 pt-14">My Favorite Books</h1>
            <div className="container mx-auto flex gap-5 py-4">
                <div className="flex gap-5 items-center text-center">
                    {favorites.map((book, index) => (
                        <div key={index} className="border p-4 rounded-lg shadow-md h-[390px] w-[235px] flex flex-col justify-between">
                            <img src={book.coverPicture} alt="book-image" className="h-[200px] w-[160px] mx-auto" />
                            <div>
                                <h2 className="text-xl font-semibold">{book.title}</h2>
                                <p className="text-gray-500">{book.genre}</p>
                                <p className="text-black font-medium">{book.author.name}</p>
                            </div>
                            <div className='flex gap-2'><Link to={`/books/${book._id}`}>
